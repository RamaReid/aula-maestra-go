      ...
      <CardContent className="space-y-5 pt-6">
        ...
        <Tabs defaultValue="fundamentacion" className="space-y-5">
          ...
          <TabsContent value="fundamentacion" className="space-y-4">
            <Card className="border-muted"><CardContent className="space-y-4 pt-5"><div className="flex items-center justify-between gap-3"><Label>Fundamentación</Label><Button type="button" variant="ghost" size="sm" onClick={() => setExpandedField("fundamentacion")}><Pencil className="mr-2 h-4 w-4" />Editar</Button></div><div className="rounded-2xl border bg-background p-5">{(splitParagraphs(plan.fundamentacion).length > 0 ? splitParagraphs(plan.fundamentacion) : ["Todavía no hay fundamentación cargada."]).map((paragraph, index) => <p key={`${paragraph}-${index}`} className="text-sm leading-7 text-foreground indent-6 [&:not(:first-child)]:mt-5">{paragraph}</p>)}</div></CardContent></Card>
          </TabsContent>
          ...
          <TabsContent value="estrategias" className="space-y-4">
            <Card><CardContent className="space-y-4 pt-5"><div className="flex items-center justify-between gap-3"><Label>Estrategia marco</Label><Button type="button" variant="ghost" size="sm" onClick={() => setExpandedField("estrategias_marco")}><Pencil className="mr-2 h-4 w-4" />Editar</Button></div><Textarea value={plan.estrategias_marco} onChange={(event) => updateField("estrategias_marco", event.target.value)} rows={5} disabled={readOnly} placeholder="Explica el enfoque metodológico general del curso." /></CardContent></Card>
